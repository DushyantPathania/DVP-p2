/* ============================================================================
   ...
   - Start with NO venue icons; highlight countries that have venues (from DB)
   - Click country -> focus + load venues for that country (cumulative)
   - Venue popup (VenueWindow) anchored to marker
   - 3D/2D toggle; leaderboard overlay
   - Bottom boxed two-thumb year slider (D3 drag, smooth)
   ============================================================================ */
...
(async function () {
  ...
  // cumulative venues across selected countries
  let venuesAll   = [];
  const venueIndex = new Set();
  let venueCountrySet = new Set();  // set of canonical country names with venues
  ...
  function canonicalName(s){ const n = norm(s); return ALIAS_TO_CANON.get(n) || n; }

  /* ---------------------------- Geography ---------------------------------- */
  function drawStaticLayers() {
    gSphere.selectAll("path.sphere").data([{ type:"Sphere" }]).join("path").attr("class","sphere").attr("d", path);
    gGraticule.selectAll("path.graticule").data([graticule]).join("path").attr("class","graticule").attr("d", path);
  }
  ...
  function applyCountryHighlight() {
    ...
    gCountries.selectAll("path.country")
      .classed("has-venues", d => venueCountrySet.has(canonicalName(d.properties?.name || "")));
  }
  ...
  async function getVenueSchema() {
    ...
    const countryCol = ["country","country_name","countrytext","nation","state"].find(c => cols.includes(c)) || null;
  ...
  /* ---------------------------- Redraw Cycle -------------------------------- */
  function redrawAll(){ gRoot.selectAll("path").attr("d", path); updateHoverTransform(); updateVenuesPosition(); }
  ...
  const zoomGlobe = d3.zoom()
    .scaleExtent([0.4, 32])    // deeper zoom in 3D
  ...
  async function loadVenueCountries(){
    ...
    if (countryCol) {
      ...
      return new Set(rows.map(r => canonicalName(r.val)));
    } else {
      const iso3Map = new Map(Object.entries({ AUS:"australia", IND:"india", PAK:"pakistan", NZL:"new zealand", ZAF:"south africa",
        LKA:"sri lanka", BGD:"bangladesh", AFG:"afghanistan", GBR:"united kingdom", ARE:"united arab emirates" }));
      const iso2Map = new Map(Object.entries({ AU:"australia", IN:"india", PK:"pakistan", NZ:"new zealand", ZA:"south africa",
        LK:"sri lanka", BD:"bangladesh", AF:"afghanistan", GB:"united kingdom", AE:"united arab emirates" }));
      const set = new Set();
      if (iso3) {
  ...
  async function loadVenuesForCountry(canonName){
    ...
    if (countryCol) {
      ...
      const nameToIso3 = new Map(Object.entries({ "australia":"AUS","india":"IND","pakistan":"PAK","new zealand":"NZL","south africa":"ZAF",
        "sri lanka":"LKA","bangladesh":"BGD","afghanistan":"AFG","united kingdom":"GBR","united arab emirates":"ARE" }));
      const nameToIso2 = new Map(Object.entries({ "australia":"AU","india":"IN","pakistan":"PK","new zealand":"NZ","south africa":"ZA",
        "sri lanka":"LK","bangladesh":"BD","afghanistan":"AF","united kingdom":"GB","united arab emirates":"AE" }));
  ...
  // start with no venues drawn, highlight countries that have venues
  ...
  // stop map drag/zoom when interacting with the year box
  ["pointerdown","mousedown","touchstart","wheel"].forEach(ev =>
